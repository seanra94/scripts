local modID = "mod_AdvancedArena";

::mods_registerMod(modID, 0.1 .0, "Advanced Arena");::mods_queue(modID, "mod_legends", function() {
    local gt = this.getroottable();

    ::mods_hookExactClass("contracts/contracts/arena_contract", function(obj) {

        obj.start = function() {
            this.buildEncounterList();
            this.setup();
            this.contract.start();
        }

        obj.buildEncounterList < - function() {
            local encounters = [];
        ...
        //local hooked_setup= obj.setup; // get the old function
        obj.setup = function() {
            //hooked_setup(); // let the old function run
            ...
            local encounters = this.encounters;

            local maxSeed = 0;

            foreach(encounter in encounters) {
                maxSeed = maxSeed + encounter.R;
            }

            local seed = this.Math.rand(1, maxSeed);

            foreach(encounter in encounters) {
                if (seed <= encounter.R) {
                    this.m.Flags.set(encounter.F, true);
                    pay = pay + encounter.P;
                    break;
                } else {
                    seed = seed - encounter.R;
                }
            }
            ...
            this.m.Screens.push({
                ...
                function start() {
                    ...
                    if (this.Flags.get("IsDirewolves")) {
                        this.Flags.set("Number", this.Math.max(2, this.Contract.getAmountToSpawn(this.Const.World.Spawn.Troops.Direwolf, baseDifficulty)));
                        this.Contract.m.BulletpointsObjectives[1] = "Enter the arena again to start the fight against %amount% direwolves";
                        this.Text += "%SPEECH_ON%Direwolveszzz%SPEECH_OFF%";
                    } else if (this.Flags.get("IsFrenziedDirewolves")) {
                        this.Flags.set("Number", this.Math.max(2, this.Contract.getAmountToSpawn(this.Const.World.Spawn.Troops.DirewolfHIGH, baseDifficulty)));
                        this.Contract.m.BulletpointsObjectives[1] = "Enter the arena again to start the fight against %amount% frenzied direwolves";
                        this.Text += "%SPEECH_ON%Frenzied Direwolves%SPEECH_OFF%";
                    }
            ...
            this.m.Screens.push({
                ...
                Options = [{
                        ...
                        function getResult() {
                            ...
                            if (this.Flags.get("IsDirewolves")) {
                                for (local i = 0; i < this.Flags.get("Number"); i = i) {
                                    this.Contract.addToCombat(p.Entities, this.Const.World.Spawn.Troops.Direwolf);
                                    i = ++i;
                                    i = i;
                                }
                            } else if (this.Flags.get("IsFrenziedDirewolves")) {
                                for (local i = 0; i < this.Flags.get("Number"); i = i) {
                                    this.Contract.addToCombat(p.Entities, this.Const.World.Spawn.Troops.DirewolfHIGH);
                                    i = ++i;
                                    i = i;
                                }
                            }
                    ...
                    if (this.World.Statistics.getFlags().getAsInt("ArenaRegularFightsWon") > 0 && this.World.Statistics.getFlags().getAsInt("ArenaRegularFightsWon") % 5 == 0) {
                        ...
                        switch (r) {
                            case 1:
                                ...
                                a.setUpgrade(this.new("scripts/items/" + (this.LegendsMod.Configs().LegendArmorsEnabled() ? "legend_armor/armor_upgrades/legend_light_gladiator_upgrade" : "armor_upgrades/light_gladiator_upgrade")));
                                this.List.push({
                                    id = 12,
                            ...
                            case 2:
                                ...
                                a.setUpgrade(this.new("scripts/items/" + (this.LegendsMod.Configs().LegendArmorsEnabled() ? "legend_armor/armor_upgrades/legend_heavy_gladiator_upgrade" : "armor_upgrades/heavy_gladiator_upgrade")));
                                this.List.push({
                                    id = 12,
